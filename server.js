...
app.get('/', (req, res) => {
    res.json({ mensagem: ' 🍔 Bem-vindo à API da Hamburgueria!' });
});
...
const rotasPedidos = require('./routers/pedidos');
...
app.use((req, res, next) => {
    res.status(404).json({
        sucesso: false,
        mensagem: `Rota '${req.url}' não encontrada na API da Hamburgueria. Verifique a URL e tente novamente.`
    });
});
...
app.listen(PORTA, () => {
    ...
    console.log(`   GET    ${PORTA}/api/pedidos/`)
    console.log('');
